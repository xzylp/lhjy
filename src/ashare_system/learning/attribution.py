...
class TradeAttributionService:
    ...
    def build_report(
        ...
        *,
        ...
        avg_return = sum(item.next_day_close_pct for item in normalized) / len(normalized)
        wins = sum(1 for item in normalized if item.next_day_close_pct >= 0.02)
    ...
    def _build_bucket_summaries(buckets: dict[str, list[TradeAttributionRecord]]) -> list[AttributionBucketSummary]:
        ...
        for key, items in buckets.items():
            positive_count = sum(1 for item in items if item.next_day_close_pct >= 0.02)
            negative_count = sum(1 for item in items if item.next_day_close_pct <= -0.02)
            ...
            summaries.append(
                AttributionBucketSummary(
                    ...
                    avg_next_day_close_pct=round(sum(item.next_day_close_pct for item in items) / len(items), 6),
                    avg_selection_score=(
